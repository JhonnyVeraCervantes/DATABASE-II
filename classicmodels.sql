...
--
-- Servidor: db
-- Tiempo de generación: 22-06-2020 a las 20:05:49
...
-- Procedimientos
--
CREATE DEFINER=`root`@`%` PROCEDURE `GetAllProducts` ()  BEGIN
SELECT * FROM products;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `GetOfficeByCountry` (IN `countryName` VARCHAR(255))  BEGIN
SELECT *
FROM offices
WHERE country = countryName;
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `GetOrderCountByStatus` (IN `orderStatus` VARCHAR(25), OUT `total` INT)  BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `MejorCliente` (`fechaInicio` DATE, `fechaFin` DATE)  BEGIN
    START TRANSACTION;
...
CREATE DEFINER=`root`@`%` PROCEDURE `newProductline` (IN `newProductline` VARCHAR(50), IN `newProductDescription` VARCHAR(4000))  BEGIN
INSERT INTO productlines (productLine,textDescription) VALUES (newProductline, newProductDescription);
END$$

CREATE DEFINER=`root`@`%` PROCEDURE `newProductlineTrans` (IN `newProductline` VARCHAR(50), IN `newProductDescription` VARCHAR(4000))  BEGIN
START TRANSACTION;
...
CREATE DEFINER=`root`@`%` PROCEDURE `OrdenarProducto` (`orderNumber` INT, `orderDate` DATE, `requiredDate` DATE, `shippedDate` DATE, `status` VARCHAR(15), `comments` TEXT, `customerNumber` INT, `productCode` VARCHAR(15), `quantityOrdered` INT, `priceEachP` DECIMAL(10,2), `orderLineNumber` SMALLINT)  BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `SetCounter` (INOUT `counter` INT, IN `inc` INT)  BEGIN
...
--
-- Estructura de tabla para la tabla `CustomersRecovery`
--
...
(1056, 'Phann', 'Mary', 'x4611', 'mpatterso@classicmodelcars.com', '1', 1002, 'VP Sales'),
...
CREATE TRIGGER `before_employee_delete` BEFORE DELETE ON `employees` FOR EACH ROW BEGIN
    INSERT INTO employeesrecords(employeeNumber, lastName, firstName, extension, email, officeCode, reportsTo, jobTitle)
    VALUES (OLD.employeeNumber, OLD.lastName, OLD.firstName, OLD.extension, OLD.email, OLD.officeCode, OLD.reportsTo, OLD.jobTitle);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_employee_update` BEFORE UPDATE ON `employees` FOR EACH ROW INSERT INTO employees_audit
    SET action         = 'update',
...
--
-- Estructura de tabla para la tabla `employeesrecords`
--
...
--
-- Estructura de tabla para la tabla `employees_audit`
--
...
--
-- Volcado de datos para la tabla `employees_audit`
--

INSERT INTO `employees_audit` (`id`, `employeeNumber`, `lastname`, `changedat`, `action`) VALUES
(1, 1056, 'Patterson', '2020-06-16 05:52:15', 'update'),
(2, 1056, 'Phan', '2020-06-16 05:53:25', 'update'),
(3, 1056, 'Phann', '2020-06-16 05:55:49', 'update');

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `logProducts`
--
...
--
-- Estructura Stand-in para la vista `mejor_cliente`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `mejor_cliente` (
`nombreCliente` varchar(50)
,`ventaTotalCliente` decimal(42,2)
,`productosDiferentes` bigint(21)
,`fechaInicio` date
,`fechaFin` date
);
...
--
-- Estructura Stand-in para la vista `mejor_cliente_compras`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `mejor_cliente_compras` (
`nombreCliente` varchar(50)
,`ventaTotalCliente` decimal(42,2)
,`productosDiferentes` bigint(21)
,`fechaInicio` date
...
CREATE TRIGGER `Before_delete_order` BEFORE DELETE ON `orders` FOR EACH ROW BEGIN
   ...
   SIGNAL SQLSTATE '45000'
   SET MESSAGE_TEXT = 'No se puede eliminar este registro de la tabla orders';
End If;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `Before_delete_payments` BEFORE DELETE ON `payments` FOR EACH ROW BEGIN
   If Old.checkNumber is not null then
   SIGNAL SQLSTATE '45000'
   SET MESSAGE_TEXT = 'No se puede eliminar este registro de la tabla payments';
End If;
END
$$
DELIMITER ;
...
('new Product', 'Descripcion del nuevo producto', NULL, NULL),
...
--
-- Disparadores `productlines`
--
DELIMITER $$
CREATE TRIGGER `before_productLines_delete` BEFORE DELETE ON `productlines` FOR EACH ROW BEGIN
...
,`utilidadProducto` decimal(33,2)
,`ventaTotal` decimal(42,2)
);
...
--
DELIMITER $$
CREATE TRIGGER `after_inserts_products` AFTER INSERT ON `products` FOR EACH ROW Begin
...
--
-- Estructura de tabla para la tabla `ProductsLinesRecovery`
--
...
--
-- Volcado de datos para la tabla `ProductsLinesRecovery`
--

INSERT INTO `ProductsLinesRecovery` (`productLine`, `textDescription`, `htmlDescription`, `image`) VALUES
('CARS', 'CAR BLACK', '', NULL);

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `ProductsRecovery`
--
...
--
-- Estructura de tabla para la tabla `Salaries_2`
--
...
--
-- Volcado de datos para la tabla `Salaries_2`
--

INSERT INTO `Salaries_2` (`employeeNumber`, `salary`) VALUES
(1056, '7000.00'),
(1076, '8000.00');

--
-- Disparadores `Salaries_2`
--
DELIMITER $$
CREATE TRIGGER `after_salaries_delete` AFTER DELETE ON `Salaries_2` FOR EACH ROW UPDATE SalaryBudgets
...
--
-- Estructura de tabla para la tabla `SalaryArchives`
--
...
--
-- Volcado de datos para la tabla `SalaryArchives`
--

INSERT INTO `SalaryArchives` (`id`, `employeeNumber`, `validFrom`, `amount`, `deletedAt`) VALUES
(1, 1002, '2000-01-01', '50000.00', '2020-06-16 05:57:09');

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `SalaryBudgets`
--
...
--
-- Volcado de datos para la tabla `SalaryBudgets`
--
...
(1, '2003 Harley-Davidson Eagle Drag Bike', 385, 2020, 1),
(2, '1969 Corvair Monza', 165, 2020, 1),
(3, '1970 Plymouth Hemi Cuda', 220, 2020, 1),
(4, '2001 Ferrari Enzo', 154, 2021, 1),
(5, '1998 Chrysler Plymouth Prowler', 121, 2021, 1),
(6, '1913 Ford Model T Speedster', 132, 2021, 1);
...
--
-- Estructura de tabla para la tabla `SalesChanges`
--
...
--
-- Volcado de datos para la tabla `SalesChanges`
--

INSERT INTO `SalesChanges` (`id`, `salesId`, `beforeQuantity`, `afterQuantity`, `changedAt`) VALUES
(1, 1, 150, 350, '2020-06-16 05:56:55'),
(2, 1, 350, 385, '2020-06-16 05:56:58'),
(3, 2, 150, 165, '2020-06-16 05:56:58'),
(4, 3, 200, 220, '2020-06-16 05:56:58'),
(5, 4, 140, 154, '2020-06-16 05:56:58'),
(6, 5, 110, 121, '2020-06-16 05:56:58'),
(7, 6, 120, 132, '2020-06-16 05:56:58');

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `WorkCenters`
--
...
--
-- Volcado de datos para la tabla `WorkCenters`
--
...
--
-- Estructura de tabla para la tabla `WorkCenterStats`
--
...
--
-- Volcado de datos para la tabla `WorkCenterStats`
--
...
--
-- Estructura para la vista `mejor_cliente`
--
DROP TABLE IF EXISTS `mejor_cliente`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mejor_cliente`  AS  select `customers`.`customerName` AS `nombreCliente`,sum((`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)) AS `ventaTotalCliente`,count(distinct `orderdetails`.`productCode`) AS `productosDiferentes`,min(`orders`.`orderDate`) AS `fechaInicio`,max(`orders`.`orderDate`) AS `fechaFin` from ((`customers` join `orderdetails`) join `orders`) where ((`customers`.`customerNumber` = `orders`.`customerNumber`) and (`orders`.`orderNumber` = `orderdetails`.`orderNumber`)) group by `customers`.`customerName` order by `productosDiferentes` desc,`fechaInicio`,`fechaFin` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `mejor_cliente_compras`
--
DROP TABLE IF EXISTS `mejor_cliente_compras`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mejor_cliente_compras`  AS  select `customers`.`customerName` AS `nombreCliente`,sum((`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)) AS `ventaTotalCliente`,count(distinct `orderdetails`.`productCode`) AS `productosDiferentes`,min(`orders`.`orderDate`) AS `fechaInicio`,max(`orders`.`orderDate`) AS `fechaFin` from ((`customers` join `orderdetails`) join `orders`) where ((`customers`.`customerNumber` = `orders`.`customerNumber`) and (`orders`.`orderNumber` = `orderdetails`.`orderNumber`)) group by `customers`.`customerName` order by `productosDiferentes` desc,`fechaInicio`,`fechaFin` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `productos_mas_vendidos`  AS  select `products`.`productName` AS `nombreProducto`,`productlines`.`productLine` AS `lineaProducto`,sum((`orderdetails`.`priceEach` - `products`.`buyPrice`)) AS `utilidadProducto`,sum((`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)) AS `ventaTotal` from ((`products` join `productlines`) join `orderdetails`) where ((`products`.`productCode` = `orderdetails`.`productCode`) and (`products`.`productLine` = `productlines`.`productLine`)) group by `products`.`productCode` order by sum(`orderdetails`.`quantityOrdered`) desc limit 1 ;
...
--
-- Indices de la tabla `CustomersRecovery`
--
ALTER TABLE `CustomersRecovery`
  ADD PRIMARY KEY (`customerNumber`);
...
--
-- Indices de la tabla `employeesrecords`
--
ALTER TABLE `employeesrecords`
  ADD PRIMARY KEY (`id`);

--
-- Indices de la tabla `employees_audit`
--
ALTER TABLE `employees_audit`
  ADD PRIMARY KEY (`id`);

--
-- Indices de la tabla `logProducts`
--
ALTER TABLE `logProducts`
...
--
-- Indices de la tabla `ProductsLinesRecovery`
--
ALTER TABLE `ProductsLinesRecovery`
  ADD PRIMARY KEY (`productLine`);

--
-- Indices de la tabla `ProductsRecovery`
--
ALTER TABLE `ProductsRecovery`
  ADD PRIMARY KEY (`productCode`);
...
--
-- Indices de la tabla `Salaries_2`
--
ALTER TABLE `Salaries_2`
  ADD PRIMARY KEY (`employeeNumber`);

--
-- Indices de la tabla `SalaryArchives`
--
...
--
-- Indices de la tabla `SalesChanges`
--
ALTER TABLE `SalesChanges`
  ADD PRIMARY KEY (`id`);

--
-- Indices de la tabla `WorkCenters`
--
...
--
ALTER TABLE `billings`
  MODIFY `billingNo` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de la tabla `employeesrecords`
--
ALTER TABLE `employeesrecords`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de la tabla `employees_audit`
--
ALTER TABLE `employees_audit`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT de la tabla `logProducts`
--
ALTER TABLE `logProducts`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT de la tabla `SalaryArchives`
--
ALTER TABLE `SalaryArchives`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
-- AUTO_INCREMENT de la tabla `SalesChanges`
--
ALTER TABLE `SalesChanges`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- AUTO_INCREMENT de la tabla `WorkCenters`
--
ALTER TABLE `WorkCenters`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
